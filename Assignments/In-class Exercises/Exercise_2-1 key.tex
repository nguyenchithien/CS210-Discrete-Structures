...
\begin{document}
    ...
    \section*{1. Turning statements to implications}
        ...
        \begin{questionNOGRADE}{\thequestion}
            ...
            \paragraph{Example:}
                When a positive integer $n$ is odd, then $n+1$ is even.

                $\forall n \in \mathbb{Z}^{+}$, if $n$ is odd, then $n+1$ is even.

            \begin{enumerate}
                \item[a.] If a shape is a square, then it has four sides that are of equal length.
                    \begin{hint}{Hint}
                        Here we can define the domain of all shapes, $S$, and come up with a variable for some shape $s$.
                        If the shape $s$ is a square, then it will have both four equal sides, AND its sides will be equal length.
                    \end{hint}

                    \iftoggle{answerkey}{ \begin{answer} If $s$ is a square, then the length of every side is $l$. \end{answer} }{ { ~\\ \raisebox{0pt}[1cm][0pt]{  } } }

                \item[b.] All prime integers are odd.   (The set of integers is $\mathbb{Z}$)
    ...
    \section*{2. Counter-examples}
        ...
        \begin{questionNOGRADE}{\thequestion}
            ...
            \begin{enumerate}
                ...
                \item[b.] For every integer $n \geq 1$, if $n$ is odd, then $n^{2} + 4$ is also prime.
    ...
    %------------------------------------------------------------------%
    \section*{5. Proving an implication}
